# Machine Learning Introduction

---

# Interview Questions

### What is Machine Learning?

Machine Learning is a branch of AI where a computer learns patterns from historical data
instead of being explicitly programmed with every rule.

The learned patterns become a trained model that can make predictions or decisions on new data.


---


### ML vs Traditional Programming

Traditional Programming

Rules + Data
      ↓
   Output


Machine Learning

Data + Answers
      ↓
Learning Algorithm
      ↓
    Model

---

### When should we use ML?

Use Machine Learning when writing explicit rules is difficult or impossible because
the problem contains complex patterns that can be learned from data.

---

### When should we not use ML?

If a problem can be solved using simple deterministic rules, traditional programming is usually faster, 
simpler, and more reliable.

---

### Give 3 real-world examples from real estate.

1. Predict property prices using historical sales data.

2. Recommend similar properties based on a user's preferences and browsing history.

3. Detect whether a property image contains amenities such as a swimming pool,
parking, or a garden using Computer Vision.

---

## Key Takeaways

- Machine Learning learns patterns from data.
- Models are learned, not manually written.
- Use ML only when writing rules is difficult.
- Data quality is as important as the algorithm.